...
parameter_names <- rownames(results_extra[[1]]$pc$credible_intervals$Gaussian_median)
  ...
  lapply(approximation_types, function(approximation_type) {
    lengths <- lapply(parameter_names, function(parameter_name) {
      all_lengths <- sapply(seq_along(results_extra), function(j) {
        length <- diff(results_extra[[j]][[prior_type]]$credible_intervals[[approximation_type]][parameter_name, ])
        length
      })
      all_lengths
    })
