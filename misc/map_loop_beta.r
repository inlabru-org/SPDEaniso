...
credible_level <- 0.05
results_beta <- vector("list", number_of_loops) # Pre-allocates a list for m iterations
...
plt_distances_to_MAP(results = results_beta, prior_types = prior_types, path = "Simulation_images/Distances_to_map_beta.png")
mean_distance_and_std_dev <- mean_distance_to_MAP_and_std_dev_of_Gaussian_approximation(results = results_beta, prior_types = prior_types)

# CI: Credible intervals
plt_CI_lengths_and_get_mean_lengths(results = results_beta, prior_types = prior_types, approximation_types = approximation_types, parameter_names = parameter_names, path = "Simulation_images/CI_lengths_beta.png")
plt_frequency_true_parameter_in_CI(results = results_beta, prior_types = prior_types, approximation_types = approximation_types, parameter_names = parameter_names, path = "Simulation_images/within_CI_beta.png")


# KL divergences
KL_approx_types <- list(importance = "importance", smoothed_importance = "smoothed_importance")
plt_KL_and_get_mean_KL(results = results_beta, prior_types = prior_types, approximation_types = KL_approx_types, path = "Simulation_images/KL_beta.png")

# PROBABILITIES that marginal posterior is smaller than true parameter
plt_probabilities(results = results_beta, prior_types = prior_types, approximation_types = approximation_types, parameter_names = parameter_names, path = "Simulation_images/probabilities_beta.png")

# KS TEST FOR EACH PARAMETER

plt_KS(results = results_beta, prior_types = prior_types, approximation_types = approximation_types, parameter_names = parameter_names, path1 = "Simulation_images/KS_distance_beta.png", path2 = "Simulation_images/KS_pvalue_beta.png")

# ks.test(x<-runif(5000),"punif")

# COMPLEXITY
plt_complexity_and_get_mean_complexity(results = results_beta, prior_types = prior_types, approximation_types = approximation_types, path = "Simulation_images/complexity_beta.png")
plt_complexity_and_get_mean_complexity(results = results_beta, prior_types = prior_types[1:2], approximation_types = approximation_types, path = "Simulation_images/Complexity_12_beta.png")
